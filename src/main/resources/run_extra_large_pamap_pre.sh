...
do
  for fi in 100
  do
  exp_name=extra_large_pamap\_$strat\_$queryId\_$ep\_$lw\_$fi
  echo " -> Starting $exp_name with argument $nodes"
  $jcmd -configurator learningWindowSize=$lw,edgePointsSize=$ep,futureInterval=$fi,proactivePAMAPPlus=$pp,queryId=$queryId,slopeActive=true
