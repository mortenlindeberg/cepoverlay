...
echo "Strat: $strat, Run: $run. QueryId: $queryId"

echo " -> Starting $exp_name with argument $nodes"
$jcmd -configurator learningWindowSize=$lw,edgePointsSize=$ep,futureInterval=$fin,proactivePAMAPPlus=$pp,queryId=$queryId
...
rm -rf run\_$run/$exp_name
cp -r $exp_name run\_$run
rm -rf $exp_name
